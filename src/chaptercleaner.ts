...
export class ChapterCleaner {
  public cleanChapter(rawChapter: string, manifest: Manifest, contentPath: string, imageroot: string, linkroot: string): string {
    ...
    rawChapter = this.removeImages(rawChapter, manifest, contentPath, imageroot);
  ...
  public removeImages(rawChapter: string, manifest: Manifest, contentPath: string, imageroot: string) {
  ...
  public replaceLinks(rawChapter: string, manifest: Manifest, contentPath: string, linkroot: string): string {
    const keys = Object.keys(manifest);
    return rawChapter.replace(/(\shref\s*=\s*["']?)([^"'\s>]*?)(["'\s>])/g, (_match, offset, str, groups) => {
      const linkparts = str && str.split("#");
      let link = linkparts.length ? [contentPath, linkparts.shift() || ""].join("/").trim() : '';
      let element;
